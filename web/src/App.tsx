...
const authBtnBase: React.CSSProperties = {
  ...
  padding: '0.625rem 1rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--line-strong)',
  cursor: 'pointer', fontSize: '0.875rem', fontWeight: 600, minHeight: 44, width: '100%',
}

function SignInButtons({ label }: { label?: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {label && <p style={{ fontSize: '0.8rem', color: 'var(--muted)', margin: 0 }}>{label}</p>}
      <button onClick={() => fas.auth.signIn('github')}
        style={{ ...authBtnBase, background: '#24292f', color: '#fff', border: 'none' }}>
      ...
      <button onClick={() => fas.auth.signIn('google')}
        style={{ ...authBtnBase, background: 'var(--paper)', color: 'var(--ink)' }}>
...
function DaySidebar({ date, events, onEventClick, onNewEvent, user }: {
  ...
  return (
    <div style={{ padding: '0.75rem', borderTop: '1px solid var(--line)', background: 'var(--paper-deep)' }}>
      ...
      {!user && dayEvents.length === 0 && (
        <div style={{ marginTop: '0.75rem' }}>
          <SignInButtons label="Sign in to sync across devices" />
        </div>
      )}
    </div>
  )
      ...
      {selectedSlot && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '0.5rem' }}>
          {user ? (
            ...
            <SignInButtons label="Sign in to book" />
          )}
        </div>
      )}
    </div>
